...
const string strMessageMagic = "DOLLAR Online Signed Message:\n";
...
int64_t GetProofOfWorkReward(int64_t nFees, int nHeight)
{
	...
	if (nHeight > 199 && nHeight < 2200)
		nSubsidy = 5000000 * COIN;

    return nSubsidy + nFees;
}

const int DAILY_BLOCKCOUNT =  1440 / 1;
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "DOLLAR Online", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "DOLLAR ONLINE";
        CTransaction txNew;
        if (!fTestNet)
        {
            txNew.nTime = 1452470300;
        }
        else
        {
            txNew.nTime    = 1452470200; // Without separate txNew.nTime near block.nTime, CheckBlock fails on coinbase timestamp for TestNet created on 14042015
        }
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1452470400;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 1498711;
        if (fTestNet)
        {
            block.nTime    = 0;
            block.nNonce   = 0;
        ...
        // debug print
        assert(block.hashMerkleRoot == uint256("0x695fc2ed5263c744a25264ed2c74a913ad327df16cfeaa587f795fadf69a5fda"));
        block.print();
